...
(function() {
    "use strict";
    PlanningHeaderCtrl.$inject = [ "$scope" ];
...
(function() {
    "use strict";
    PlanningCtrl.$inject = [ "$scope", "InfinitePaging" ];
...
angular.module("itaca.components").run([ "$templateCache", function($templateCache) {
    ...
    $templateCache.put("/tpls/planning/planning-header.tpl", '<div class="ch-planning-header" flex layout layout-padding ng-style="background-image: $ctrl.$$bgImage"><div flex><ch-month-picker ng-model="$ctrl.$$startDate" hide-icon="true" ng-change="$ctrl.$onMonthChange()"></ch-month-picker></div><div><md-button class="md-icon-button" ng-click="$ctrl.$goToToday()" aria-label="Go to today"><md-icon class="mdi mdi-calendar-today md-24"></md-icon></md-button></div><div><md-menu><md-button ng-click="$mdMenu.open($event)" aria-label="Switch view"><span ng-switch="$ctrl.$$currentView"><span ng-switch-when="D" translate="date.day"></span><span ng-switch-when="W" translate="date.week"></span><span ng-switch-when="M" translate="date.month"></span></span></md-button><md-menu-content><md-menu-item><md-button ng-click="$ctrl.$dailyView()" aria-label="Switch to daily view"><span translate="date.day"></span></md-button></md-menu-item><md-menu-item><md-button ng-click="$ctrl.$weeklyView()" aria-label="Switch to weekly view"><span translate="date.week"></span></md-button></md-menu-item><md-menu-item><md-button ng-click="$ctrl.$monthlyView()" aria-label="Switch to monthly view"><span translate="date.month"></span></md-button></md-menu-item></md-menu-content></md-menu></div><div><md-menu><md-button class="md-icon-button" ng-click="$mdMenu.open($event)" aria-label="Open planning menu"><md-icon class="mdi mdi-dots-vertical md-24"></md-icon></md-button><md-menu-content><md-menu-item><md-button ng-click="$ctrl.$doSomething()" aria-label="Do something">Do Something</md-button></md-menu-item></md-menu-content></md-menu></div></div>');
    $templateCache.put("/tpls/planning/planning.tpl", '<div flex layout="column" layout-fill><ch-planning-header></ch-planning-header><div flex layout="column" ng-switch="$ctrl.$$currentView"><md-content ng-switch-when="W" flex id="planningCont" ng-switch="$ctrl.$$roomsType"><md-subheader class="no-padding bg-gray-lighter"><div layout flex><div hide show-gt-sm flex="25" layout layout-padding layout-align="center center" class="border-right-white"><strong class="text-uppercase text-gray-light"><span translate-once="room.room"></span></strong></div><div flex layout="column"><div layout layout-padding-sm class="no-padding-right no-padding-left"><div flex layout="column" class="border-right-white" ng-repeat="viewDate in $ctrl.$$viewDates track by viewDate.uid"><md-button tabindex="-1" ng-click="$ctrl.toggleAllRoomsClosing(viewDate.date)" ng-disabled="viewDate.hotelStatus.hotelClosed" class="no-margin row-mini" ng-class="{\'minimal-button\': !$ctrl.$mdMedia(\'gt-xs\'),\n' + "\t\t\t\t\t\t\t\t\t'text-gray-light': !viewDate.hotelStatus.roomsClosed,\n" + '\t\t\t\t\t\t\t\t\t\'bg-danger text-white\': viewDate.hotelStatus.roomsClosed}"><div layout="column"><small class="text-capitalize">{{viewDate.date|date:"EEE"}}</small><div><strong>{{viewDate.date|date:"dd"}}</strong></div><small hide show-gt-sm class="text-capitalize">{{viewDate.date|date:"MMM"}}</small></div><md-tooltip><span ng-show="viewDate.hotelStatus.roomsClosed" translate-once="hotel.availability.open.day"></span><span ng-show="!viewDate.hotelStatus.roomsClosed" translate-once="hotel.availability.close.day"></span></md-tooltip></md-button></div></div></div></div></md-subheader><div ng-switch-when="2" infinite-scroll="$ctrl.rooms.nextPage()" infinite-scroll-container="\'#planningCont\'" infinite-scroll-disabled="$ctrl.rooms.busy" infinite-scroll-distance="1"><div class="layout-padding no-padding" ng-repeat="room in $ctrl.rooms.items track by room.id"><div class="no-padding-bottom no-padding-x-sides" layout-padding-sm><md-subheader class="md-no-sticky no-padding"><div layout-gt-sm="row" layout-sm="column" layout-xs="column"><div flex-xs="100" flex-sm="100" flex-gt-sm="25" layout="column" layout-padding-sm><div flex layout="column" layout-align="center center"><strong class="text-primary text-uppercase text-center no-padding-bottom" ng-bind="room.name"></strong></div></div><span ng-if="!room.enabled" class="locked-string bg-warn opaque text-uppercase"><span translate-once="common.disabled.female"></span></span><div flex layout="column"><div layout layout-padding-sm layout-align="center center"><div class="flex border-right-white no-padding" ng-repeat="header in rtRatesheet.headers track by header.date.getTime()"><ch-ratesheet-header header="header" on-toggle-closing="$ctrl.toggleRoomTypeClosing(rtRatesheet.roomType.id, $date, $closed)"></ch-ratesheet-header></div></div></div></div></md-subheader></div></div></div><div ng-switch-default></div></md-content></div></div>');
