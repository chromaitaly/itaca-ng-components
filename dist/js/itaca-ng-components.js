...
angular.module("itaca.components").run([ "$templateCache", function($templateCache) {
    ...
    $templateCache.put("/tpls/color-picker/color-picker-button.tpl", '<md-button class="ch-color-picker-button" ng-style="$ctrl.$$btnStyle" ng-disabled="$ctrl.ngDisabled" ng-click="$ctrl.$openPicker($event)" aria-label="{{$ctrl.label || \'Select color\'}}"><span ng-if="!$ctrl.ngModel" ng-bind="$ctrl.label"></span></md-button>');
