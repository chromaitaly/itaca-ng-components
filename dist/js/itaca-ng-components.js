    ...
    function HotelMapMarkerCtrl($scope) {
        ...
        scope.getMarkerPosition = function() {
            ...
            if (ctrl.hotel.addressInfo.geo && !_.isEmpty(ctrl.hotel.addressInfo.geo.coordinates)) {
                ctrl.$$position = new google.maps.LatLng({
                    lat: ctrl.hotel.addressInfo.geo.coordinates[1],
                    lng: ctrl.hotel.addressInfo.geo.coordinates[0]
                });
            } else {
        ...
        this.$getUserAvatar = function() {
            if (ctrl.$$user && ctrl.$$user.avatarType) {
                switch (ctrl.$$user.avatarType) {
                  ...
                  case "FACEBOOK":
                    ctrl.userAvatar = ctrl.$$user.facebookImage;
                    break;

                  case "GOOGLE":
                    ctrl.userAvatar = ctrl.$$user.googleImage;
                    break;
...
(function() {
    "use strict";
    RoomEditBedsCtrl.$inject = [ "$scope", "$translate", "$mdMedia", "Dialog", "ReservationUtils", "DateUtils", "NumberUtils" ];
    ...
    function RoomEditBedsCtrl($scope, $translate, $mdMedia, Dialog, ReservationUtils, DateUtils, NumberUtils) {
        ...
        this.$toggleDetails = function(show) {
            ...
            ctrl.onToggleConfig && ctrl.onToggleConfig({
                $open: ctrl.$$showDetails
            });
            ctrl.$manageBeds();
        ...
        this.$removeBed = function(bedSold) {
            ...
            if (!_.isNil(removed)) {
                ctrl.onRemove && ctrl.onRemove({
                    $bed: !_.isArray(removed) ? removed : _.size(removed) == 1 ? removed[0] : removed
                });
            }
        };
...
(function() {
    "use strict";
    RoomEditIdentityDocumentsCtrl.$inject = [ "$scope", "$mdMedia", "$translate", "AppOptions", "Notification", "Navigator", "Loading", "GuestDetailsAPI", "NumberUtils", "ReservationUtils" ];
    ...
    function RoomEditIdentityDocumentsCtrl($scope, $mdMedia, $translate, AppOptions, Notification, Navigator, Loading, GuestDetailsAPI, NumberUtils, ReservationUtils) {
        ...
        this.$adeguateGuestType = function(document) {
            ...
            if (_.isNil(ctrl.$checkIsAnLeader())) {
                _.forEach(ctrl.reservation.rooms, function(room) {
                    _.forEach(room.identityDocuments, function(d) {
                        d.guestType = "SINGLE_GUEST";
                    });
                });
            }
            ctrl.room.identityDocuments = _.orderBy(ctrl.room.identityDocuments, [ function(doc) {
                return doc.guestType == "GROUP_LEADER" || doc.guestType == "HOUSEHOLDER" ? 0 : 1;
            }, "name", "surname" ], [ "asc" ]);
            ctrl.$checkDoc();
        };
...
(function() {
    "use strict";
    RoomEditPeopleCtrl.$inject = [ "$scope", "ReservationUtils" ];
...
(function() {
    "use strict";
    RoomEditServicesCtrl.$inject = [ "$scope", "$translate", "$mdMedia", "Lang", "IconUtils", "Dialog", "ReservationUtils", "DateUtils", "NumberUtils" ];
    ...
    function RoomEditServicesCtrl($scope, $translate, $mdMedia, Lang, IconUtils, Dialog, ReservationUtils, DateUtils, NumberUtils) {
        ...
        this.$removeService = function(serviceSold) {
            ...
            if (!_.isNil(removed)) {
                ctrl.onRemove && ctrl.onRemove({
                    $service: !_.isArray(removed) ? removed : _.size(removed) == 1 ? removed[0] : removed
                });
            }
        };
...
(function() {
    "use strict";
    RoomEditCtrl.$inject = [ "$scope", "$mdMedia", "$translate", "IconUtils", "NumberUtils", "Dialog", "ReservationUtils", "Navigator" ];
    ...
    function RoomEditCtrl($scope, $mdMedia, $translate, IconUtils, NumberUtils, Dialog, ReservationUtils, Navigator) {
        ...
        this.$onPeopleChange = function(people) {
            ...
            ctrl.$managePeople(ctrl.room.people, ctrl.room.extraPeople);
            ctrl.onPeopleChange && ctrl.onPeopleChange({
                $people: people
            });
        };
        this.$onExtraPeopleChange = function(people) {
            ...
            ctrl.$managePeople(ctrl.room.people, ctrl.room.extraPeople);
            ctrl.onExtraPeopleChange && ctrl.onExtraPeopleChange({
                $people: people
            });
        };
...
angular.module("itaca.components").run([ "$templateCache", function($templateCache) {
    "use strict";
    $templateCache.put("/tpls/amount-input/amount-input.tpl", '<ng-form name="chAmountInputForm"><div layout layout-padding layout-align="start top" class="no-padding"><div><md-input-container md-no-float="$ctrl.labelNoFloat" class="{{$ctrl.inputContainerClass}}" ng-class="{\'md-icon-left\': !$ctrl.hideIcon && $ctrl.amountType, \'no-padding-left\': $ctrl.hideIcon || !$ctrl.amountType}"><label ng-if="$ctrl.label && !$ctrl.labelNoFloat"><span ng-bind="$ctrl.label"></span></label><md-icon ng-if="!$ctrl.hideIcon && $ctrl.amountType == \'PRICE\'" class="mdi md-18" ng-class="{\n' + "\t\t\t\t\t'mdi-currency-eur': !$ctrl.amountCurrency || $ctrl.amountCurrency == 'EUR', \n" + '\t\t\t\t\t\'mdi-currency-usd\': $ctrl.amountCurrency == \'USD\'}"></md-icon><md-icon ng-if="!$ctrl.hideIcon && $ctrl.amountType == \'PERCENTAGE\'" class="mdi mdi-percent md-18"></md-icon><input type="number" name="{{$ctrl.inputName}}" placeholder="{{$ctrl.label}}" ng-model="$ctrl.ngModel.finalAmount" ng-required="$ctrl.ngRequired" ng-readonly="$ctrl.readonly" ng-change="$ctrl.ngChange()" ng-disabled="$ctrl.ngDisabled" ng-min="$ctrl.ngMin" min="{{$ctrl.ngMin}}" ng-step="$ctrl.ngStep" step="{{$ctrl.ngStep}}" ng-max="($ctrl.ngModel.type == \'PERCENTAGE\' ? 100 : $ctrl.ngMax)" max="{{($ctrl.ngModel.type == \'PERCENTAGE\' ? 100 : $ctrl.ngMax)}}" ng-pattern="($ctrl.allowNegative ? $ctrl.$$REGEXP.priceNoStrict : $ctrl.$$REGEXP.price)" aria-label="Amount"><div ng-if="$ctrl.errorMessages" ng-messages="chAmountInputForm[$ctrl.inputName].$error"><div ng-repeat="errorObj in $ctrl.errorMessages" ng-message="{{errorObj.error}}"><span ng-if="errorObj.message" ng-bind="{{errorObj.message}}"></span><span ng-if="!errorObj.message && errorObj.messageKey" translate="{{errorObj.messageKey}}" translate-values="errorObj.messageKeyParams"></span></div></div></md-input-container></div><div ng-if="!$ctrl.amountType"><md-input-container md-no-float class="{{$ctrl.inputContainerClass}}"><md-select name="amountType" placeholder="{{::(\'common.amount.type\'|translate)}}" ng-model="$ctrl.ngModel.type" ng-required="$ctrl.ngRequired" ng-readonly="$ctrl.readonly" ng-change="$ctrl.ngChange()" aria-label="Amount type" ng-disabled="$ctrl.ngDisabled"><md-option value="PRICE" ng-selected="!$ctrl.ngModel.type || $ctrl.ngModel.type == \'PRICE\'"><md-icon class="mdi md-14" ng-class="{\n' + "\t         \t\t\t\t'mdi-currency-eur': !$ctrl.amountCurrency || $ctrl.amountCurrency == 'EUR', \n" + '\t         \t\t\t\t\'mdi-currency-usd\': $ctrl.amountCurrency == \'USD\'}"></md-icon></md-option><md-option value="PERCENTAGE" ng-selected="$ctrl.ngModel.type == \'PERCENTAGE\'"><md-icon class="mdi mdi-percent md-14"></md-icon></md-option></md-select><div ng-messages="chAmountInputForm.amountType.$error"><div ng-message="required"><span translate-once="error.required"></span></div></div></md-input-container></div></div></ng-form>');
    ...
    $templateCache.put("/tpls/cancellation-policy-info/cancellation-policy-info.tpl", '<div><h4 class="{{$ctrl.titleClass}}"><strong><span ng-if="!$ctrl.title" translate="reservation.cancellation.terms.conditions"></span><span ng-if="$ctrl.title" ng-bind="$ctrl.title"></span></strong></h4><div ng-if="!$ctrl.cancellationPolicy || !$ctrl.cancellationPolicy.cancellation"><span translate="reservation.cancellation.room.no.condition"></span></div><div flex ng-if="$ctrl.cancellationPolicy && $ctrl.cancellationPolicy.cancellation"><div class="layout-column" ng-if="!$ctrl.cancellationPolicy.cancellation.deposit"><div ng-switch="$ctrl.rateType"><div ng-switch-when="STANDARD"><span ng-if="!$ctrl.cancellationPolicy.flexible"><span translate="reservation.cancellation.room.condition.free.label"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.limitDate|date:\'shortDate\'"></span>&nbsp;</strong><span translate="date.time.to" class="text-lowercase"></span><span>&nbsp;<strong><span ng-bind="$ctrl.cancellationPolicy.limitDate|offsetDate:\'HH:mm\':$ctrl.offset"></span></strong>&nbsp;</span><span>[<span ng-bind="$ctrl.city"></span>].&nbsp;</span><span translate="reservation.cancellation.room.condition.free.label2"></span></span><span ng-if="$ctrl.cancellationPolicy.flexible"><span translate="reservation.cancellation.room.condition.flex.label" translate-values="{limit: (($ctrl.cancellationPolicy.limitDate|offsetDate:\'short\':$ctrl.offset) + \' [\' + $ctrl.city + \']\')}"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights != $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == 1"><span translate="reservation.percentage.firstNight"></span>&nbsp;<span translate="common.night" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights > 1"><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights < reservation.nights"><span translate="reservation.percentage.nights"></span>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.chargeNights"></span>&nbsp;<span translate="common.nights" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights >= reservation.nights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="!$ctrl.cancellationPolicy.cancellation.chargeNights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span translate="reservation.percentage.total.stay"></span></span>&nbsp;<span translate="reservation.total.night"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.amount.finalAmount|chCurrency"></span></strong>.</div><div ng-switch-when="NOT_REFUNDABLE"><span translate="reservation.cancellation.room.condition.notRef.label"></span><span ng-if="$ctrl.cancellationPolicy.limitDate"><span translate="reservation.cancellation.room.condition.notRef.label2"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.limitDate|date:\'shortDate\'"></span>&nbsp;</strong><span translate="date.time.to" class="text-lowercase"></span><span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.limitDate|offsetDate:\'HH:mm\':$ctrl.offset"></span>&nbsp;</strong></span><span>[<span ng-bind="$ctrl.city"></span>],</span><span translate="reservation.cancellation.room.condition.notRef.label3"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights != $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == 1"><span translate="reservation.percentage.firstNight"></span>&nbsp;<span translate="common.night" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights > 1"><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights < reservation.nights"><span translate="reservation.percentage.nights"></span><span ng-bind="$ctrl.cancellationPolicy.cancellation.chargeNights"></span>&nbsp;<span translate="common.nights" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights >= reservation.nights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="!$ctrl.cancellationPolicy.cancellation.chargeNights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span translate="reservation.percentage.total.stay"></span></span>&nbsp;<span translate="reservation.total.night"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.amount.finalAmount|chCurrency"></span></strong>.</span><span ng-if="!$ctrl.cancellationPolicy.limitDate">&nbsp;<span translate="reservation.cancellation.room.condition.notRef.label4"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.amount.finalAmount|chCurrency"></span></strong>.</span></div></div></div><div class="layout-column" ng-if="$ctrl.cancellationPolicy.cancellation.deposit"><div class="no-margin-top"><span translate="reservation.room.deposit.label1"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights != $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == 1"><span translate="reservation.percentage.firstNight"></span>&nbsp;<span translate="common.night" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights > 1"><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights < reservation.nights"><span translate="reservation.percentage.nights"></span>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.chargeNights"></span>&nbsp;<span translate="common.nights" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights >= reservation.nights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="!$ctrl.cancellationPolicy.cancellation.chargeNights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span translate="reservation.percentage.total.stay"></span></span>&nbsp;<span translate="reservation.total.night"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.amount.finalAmount|chCurrency"></span>&nbsp;</strong>.<span translate="reservation.room.deposit.label2"></span></div></div></div></div>');
    ...
    $templateCache.put("/tpls/no-show-policy-info/no-show-policy-info.tpl", '<div><h4 class="{{$ctrl.titleClass}}"><strong><span ng-if="!$ctrl.title" translate="reservation.noshow.terms.conditions"></span><span ng-if="$ctrl.title" ng-bind="$ctrl.title"></span></strong></h4><div ng-if="!$ctrl.noShowPolicy || !$ctrl.noShowPolicy.cancellation"><span translate="reservation.cancellation.room.no.condition"></span></div><div flex ng-if="$ctrl.noShowPolicy && $ctrl.noShowPolicy.cancellation"><div><span translate="reservation.noshow.room.conditions"></span><span ng-if="$ctrl.noShowPolicy.cancellation.chargeNights != $ctrl.noShowPolicy.cancellation.stayNights"><span ng-bind="$ctrl.noShowPolicy.cancellation.percentage"></span><span ng-if="$ctrl.noShowPolicy.cancellation.chargeNights == 1"><span translate="reservation.percentage.firstNight"></span><span translate="common.night" class="text-lowercase"></span></span><span ng-if="$ctrl.noShowPolicy.cancellation.chargeNights > 1"><span ng-if="$ctrl.noShowPolicy.cancellation.chargeNights < reservation.nights"><span translate="reservation.percentage.nights"></span><span ng-bind="$ctrl.noShowPolicy.cancellation.chargeNights"></span>&nbsp;<span translate="common.nights" class="text-lowercase"></span></span><span ng-if="$ctrl.noShowPolicy.cancellation.chargeNights >= reservation.nights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="!$ctrl.noShowPolicy.cancellation.chargeNights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="$ctrl.noShowPolicy.cancellation.chargeNights == $ctrl.noShowPolicy.cancellation.stayNights"><span ng-bind="$ctrl.noShowPolicy.cancellation.percentage"></span><span translate="reservation.percentage.total.stay"></span></span><span translate="reservation.total.night"></span><strong><span ng-bind="$ctrl.noShowPolicy.amount.finalAmount|chCurrency"></span></strong>.</div></div></div>');
    ...
    $templateCache.put("/tpls/password-input/password-input.tpl", '<ng-form name="chPasswordInputForm" flex><md-input-container class="{{$ctrl.containerClass}} no-padding-right" ng-class="{\'no-padding-left\': !$ctrl.iconClass, \'md-icon-left\': $ctrl.iconClass, \'md-icon-right\': $ctrl.showIconClass || $ctrl.hideIconClass}"><label><span ng-if="$ctrl.inputLabel" ng-bind="$ctrl.inputLabel"></span><span ng-if="!$ctrl.inputLabel" translate="login.password"></span></label><md-icon ng-if="$ctrl.iconClass" class="{{$ctrl.iconClass}}"></md-icon><input type="{{$ctrl.$$showPwd ? \'text\' : \'password\'}}" name="{{$ctrl.inputName}}" ng-model="$ctrl.ngModel" autocomplete="{{$ctrl.autocomplete}}" ng-pattern="$ctrl.ngPattern" ng-required="$ctrl.ngRequired" ng-disabled="$ctrl.ngDisabled" ng-readonly="$ctrl.ngReadonly" ng-change="$ctrl.ngChange()" style="padding-right: 30px"><md-icon class="clickable" ng-class="$ctrl.$$showPwd ? $ctrl.hideIconClass : $ctrl.showIconClass" ng-click="$ctrl.$$showPwd = !$ctrl.$$showPwd" aria-label="Show/Hide password" tabindex="-1"><md-tooltip><span ng-show="$ctrl.$$showPwd" translate="common.hide"></span><span ng-show="!$ctrl.$$showPwd" translate="common.show"></span></md-tooltip></md-icon><div ng-messages="chPasswordInputForm[$ctrl.inputName].$error"><div ng-repeat="errorObj in $ctrl.errorMessages" ng-message="{{errorObj.error}}"><span ng-if="errorObj.message" ng-bind="{{errorObj.message}}"></span><span ng-if="!errorObj.message && errorObj.messageKey" translate="{{errorObj.messageKey}}" translate-values="errorObj.messageKeyParams"></span></div></div></md-input-container></ng-form>');
    ...
    $templateCache.put("/tpls/payment-policy-info/payment-policy-info.tpl", '<div><h4 class="{{$ctrl.titleClass}}"><strong><span ng-if="!$ctrl.title" translate="common.payment"></span><span ng-if="$ctrl.title" ng-bind="$ctrl.title"></span></strong></h4><div ng-if="!$ctrl.cancellationPolicy"><span translate="reservation.payment.room.policy.free.label"></span>&nbsp;<span translate="reservation.payment.room.policy.free.label2"></span></div><div flex ng-if="$ctrl.cancellationPolicy"><div ng-if="!$ctrl.cancellationPolicy.cancellation.deposit && (!$ctrl.$$charged.payed || $ctrl.$$charged.payed <= 0) && $ctrl.paymentMethod != \'PAYPAL\'"><div ng-if="$ctrl.rateType == \'STANDARD\' && !$ctrl.cancellationPolicy.flexible"><span translate="reservation.payment.room.policy.free.label"></span>&nbsp;<span translate="reservation.payment.room.policy.free.label2"></span>&nbsp;<span translate="reservation.payment.room.policy.free.label3"></span></div><div ng-if="$ctrl.rateType == \'NOT_REFUNDABLE\' || $ctrl.cancellationPolicy.flexible"><span translate="reservation.payment.room.policy.notRef.label"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights != $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == 1"><span translate="reservation.percentage.firstNight"></span>&nbsp;<span translate="common.night" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights > 1"><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights < reservation.nights"><span translate="reservation.percentage.nights"></span>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.chargeNights"></span>&nbsp;<span translate="common.nights" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights >= reservation.nights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="!$ctrl.cancellationPolicy.cancellation.chargeNights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span translate="reservation.percentage.total.stay"></span></span>&nbsp;<span translate="reservation.total.night"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.amount.finalAmount|chCurrency"></span>&nbsp;</strong><br><span ng-if="!$ctrl.cancellationPolicy.limitDate || $ctrl.cancellationPolicy.flexible" translate="reservation.payment.room.policy.notRef.label2"></span><span ng-if="$ctrl.cancellationPolicy.limitDate && !$ctrl.cancellationPolicy.flexible"><span translate="reservation.payment.room.policy.notRef.label2.bis"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.limitDate|date:\'shortDate\'"></span>&nbsp;</strong><span translate="date.time.to" class="text-lowercase"></span><span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.limitDate|offsetDate:\'HH:mm\':$ctrl.offset"></span>&nbsp;</strong></span><span>[<span ng-bind="$ctrl.city"></span>].</span></span></div></div><div class="layout-column" ng-if="$ctrl.cancellationPolicy.cancellation.deposit && (!$ctrl.$$charged.payed || $ctrl.$$charged.payed <= 0) && $ctrl.paymentMethod != \'PAYPAL\'"><span class="no-margin-top"><span translate="reservation.payment.room.policy.deposit.label"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights != $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == 1"><span translate="reservation.percentage.firstNight"></span>&nbsp;<span translate="common.night" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights > 1"><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights < reservation.nights"><span translate="reservation.percentage.nights"></span>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.chargeNights"></span>&nbsp;<span translate="common.nights" class="text-lowercase"></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights >= reservation.nights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="!$ctrl.cancellationPolicy.cancellation.chargeNights"><span translate="reservation.percentage.total.stay"></span></span></span><span ng-if="$ctrl.cancellationPolicy.cancellation.chargeNights == $ctrl.cancellationPolicy.cancellation.stayNights">&nbsp;<span ng-bind="$ctrl.cancellationPolicy.cancellation.percentage"></span><span translate="reservation.percentage.total.stay"></span></span>&nbsp;<span translate="reservation.total.night"></span><strong>&nbsp;<span ng-bind="$ctrl.cancellationPolicy.amount.finalAmount|chCurrency"></span>&nbsp;</strong><span translate="reservation.payment.room.policy.notRef.label2"></span></span></div><div ng-if="$ctrl.$$charged.payed > 0 && $ctrl.paymentMethod == \'PAYPAL\'"><span ng-if="$ctrl.$$charged.tbp > 0" translate="reservation.payment.room.policy.charged.partial.label" traslate-values="{payed: ($ctrl.$$charged.payed|chCurrency), tbp: ($ctrl.$$charged.tbp|chCurrency}"></span><span ng-if="$ctrl.$$charged.tbp <= 0" translate="reservation.payment.room.policy.charged.total.label"></span></div></div></div>');
